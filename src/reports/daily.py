def generate_report(ranked, symbol, tech_score, backtest_metrics=None):
    ...
    lines = ['# Daily AI Crypto Research Report', '', '## Market Leaders by Score']
    for item in top:
    ...
    for note in tech_score.get('notes', []): lines.append(f'- {note}')
    if backtest_metrics:
        lines += ['', '## Backtest Snapshot']
        for k, v in backtest_metrics.items(): lines.append(f'- {k}: {v}')
    lines += ['', '## Decision Discipline', '- Define entry, invalidation, target, and position size before any trade.', '- Avoid leverage unless you have tested the strategy over several market regimes.', '- This report is research support, not financial advice.']
    return '
'.join(lines)
